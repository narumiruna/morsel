...
```ts
import { MorselApiError, MorselClient } from "@narumiruna/morsel"
...
To consume a share, pass the 43-character capability token (from the URL after `/s/` or `#/s/`) to `consumeShare(token)`. No API key is needed for reading. **Every successful call consumes one view**; the SDK never caches or retries a request. Each method accepts an optional `{ signal }` for cancellation.

The package lives in `sdk/` and is not published automatically. From that directory, run `npm ci && npm run ci` and inspect `npm pack --dry-run` before publishing. The request and response types follow the [OpenAPI contract](https://github.com/narumiruna/morsel/blob/main/api/openapi.yaml).
