...
test("createShare sends Markdown and options using server-side Bearer auth", async () => {
  ...
  const body = { content: "# Hello", expires_in: 3600, max_views: 3, preview: { title: "Hi", description: "Hello" } }
  const controller = new AbortController()
  const result = await client.createShare(body, { signal: controller.signal })
  assert.equal(result.share_url, `${origin}/#/s/${token}`)
  assert.deepEqual(calls, [[`${origin}/v1/shares`, {
    method: "POST",
...
test("consumeShare does not send credentials, cache, or retry", async () => {
  const calls = []
  const client = new MorselClient({ baseUrl: origin, apiKey: "secret", fetch: async (...args) => {
    calls.push(args)
    return json({ content: "Hello", created_at: "2026-01-01T00:00:00Z", view_count: calls.length }, 200)
  } })
  ...
  assert.deepEqual(calls, Array.from({ length: 2 }, () => [
    ...
    { method: "GET", headers: { Accept: "application/json" }, cache: "no-store", credentials: "omit", referrerPolicy: "no-referrer", signal: undefined },
  ]))
})
...
test("revokeShare uses administrative UUID and expects an empty 204 response", async () => {
  ...
  assert.deepEqual(calls, [[`${origin}/v1/shares/${id}`, {
    method: "DELETE", headers: { Authorization: "Bearer secret" }, signal: undefined,
  }]])
})

test("creating or revoking without an API key fails before fetch; reading remains public", async () => {
  let requests = 0
  const client = new MorselClient({ baseUrl: origin, fetch: async () => {
    requests++
...
test("API errors preserve status, code and message", async () => {
  const client = new MorselClient({ baseUrl: origin, fetch: async () => json({ code: "view_limit_exhausted", message: "gone" }, 410) })
...
test("proxy and malformed responses produce useful errors without leaking keys", async () => {
  const client = new MorselClient({ baseUrl: origin, apiKey: "secret", fetch: async () => new Response("bad gateway", { status: 502 }) })
  await assert.rejects(client.createShare({ content: "Hello" }), { status: 502, code: "unknown", message: "HTTP 502" })
  const invalid = new MorselClient({ baseUrl: origin, fetch: async () => new Response("not json", { status: 200 }) })
  await assert.rejects(invalid.consumeShare(token), { status: 200, code: "unknown", message: "Invalid JSON response" })
})
...
test("rejects URLs that could misroute API requests or contain credentials", () => {
  for (const baseUrl of ["https://user:pass@example.com", "https://example.com/subpath", "https://example.com/?q=1", "https://example.com/#fragment", "file:///tmp/foo"]) {
